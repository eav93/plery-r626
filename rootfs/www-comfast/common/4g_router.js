define(function (require, exports) {
	...
	//'dispals' Change events
	et.dispals = function () {
		displays()
	}
	function displays() {
		var apn_auth = d('#apn_auth').val()
		if (apn_auth == 0) {//如果APN鉴权下标为0
			d("#usernamebox").addClass('siapals')
			d("#passwordbox").addClass('siapals')
		} else if (apn_auth != 0) {//否则取反
			d("#usernamebox").removeClass('siapals')
			d("#passwordbox").removeClass('siapals')
		}
	}
	displays()
	//Next event
	et.next = function (evt) {
		var lte_password = d("#lte_password").val()
		if (/[\u4E00-\u9FA5]/g.test(lte_password)) {//Cannot input Chinese character regularization
			g.shconfirm(nowLang, SHtips.chinese[nowLang], "error");
		} else {
			ifNext(evt);
		}
	}
	...
	function ifNext(obj, callback) {
		d("#lte_username").removeClass('borError require')
					d("#lte_password").removeClass('borError require')
		var apn_auth = d('#apn_auth').val()
		var lte_username = d("#lte_username").val();
		var lte_password = d("#lte_password").val();
		...
		if (d('#apn_auth').val() == 0) {
			if (errobj.length == 0) {
				...
				d(errobj[0]).trigger('blur').focus(); //APN cannot be empty!
			}
		} else {
			if(d("input[name='addr_ip']:checked").val() == 'qmi' && d('#apn_detect').val() == 0) {
				if (lte_username == '' && lte_password == "") {//Validation is not empty
					d("#lte_username").addClass('borError require')
					d("#lte_password").addClass('borError require')
					g.shconfirm(nowLang, SHtips.no_null_name[nowLang], "error");
				} else if (lte_username == '') {
					d("#lte_username").addClass('borError')
					g.shconfirm(nowLang, SHtips.namen_ull[nowLang], "error");
				} else if (lte_password == "") {
					d("#lte_password").addClass('borError require')
					g.shconfirm(nowLang, SHtips.pwd_null[nowLang], "error");
				} else {
					d("#lte_username").removeClass('borError require')
					d("#lte_password").removeClass('borError require')
					...
					showPics(num, callback)//callback function
				}
	...
	et.country_channel = function (evt) {
		...
		d.each(rwinfo, function (n, m) {
			h.append_channel("#channels_" + m.flag, now_country, "auto", m.flag, d('#bandwidth_' + m.flag).val(), m.no_ht80_with_11a,
				0);
		})
	};
	...
	et.htmode_channel = function (evt) {
		d.each(rwinfo, function (n, m) {
			if (m.flag == '58g') {
				h.append_channel("#channels_" + m.flag, d('.country').val(), "auto", m.flag, d(evt).val(), m.no_ht80_with_11a,
					0);
			}
		})
	};
	...
	function refresh_default() {
		...
		} else if (wan_info.proto == 'qmi' && wan_info.apn_detect === '0') {
			d('#apn_msg').removeClass('hidden');
			d("#apn_detect").val(wan_info.apn_detect);
			d("#apn_auth").val(wan_info.apn_auth);
	...
	function config_get() {
		...
		data_config.wan.proto = d("input[name='addr_ip']:checked").val();
		if (data_config.wan.proto == "pppoe") {
		...
		} else if (data_config.wan.proto == "static") {

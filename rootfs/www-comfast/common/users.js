define(function (require, exports) {
    ...
    var device, macfilter_info, dhcp_status, arp_list, dhcp_info, assoc_info, filter_data, filter_type, filter_info = [], assoc_array = [], filter_array, set_num = [], now_num, optflag;
    ...
    function refresh_init() {
        ...
        f.getWifiAssoc(function (data) {
            //data = {"wifi-iface":[[{"iface":0,"radio":"radio0"},{"mac":"20:08:ED:35:B9:9B","contime":27841,"rxbytes":400282,"txbytes":133469},{"mac":"20:08:ED:35:B9:9B","contime":27841,"rxbytes":400282,"txbytes":133469},{"mac":"20:08:ED:35:B9:9B","contime":27841,"rxbytes":400282,"txbytes":133469}],[{"iface":1,"radio":""}],[{"iface":2,"radio":""}],[{"iface":3,"radio":""}],[{"iface":4,"radio":""}],[{"iface":5,"radio":""}],[{"iface":6,"radio":""}],[{"iface":7,"radio":"radio0"}]],"errCode":0,"errMsg":"OK","configDone":false};
            assoc_info = [];
            if (data.errCode == 0) {
        ...
        d.each(assoc_info, function (n, m) {
            if (m && m.length > 1 && m.ifname != 'vif-sta0') {
                d.each(m, function (x, y) {
                    if (x != 0) {
                        for (var mnum = 0; mnum < macfilter_info.length; mnum++) {
                            var macdata = macfilter_info[mnum];
                            if (macdata.src_mac == y.mac.toLowerCase()) {
                                return true;
                            }
                        }
    ...
    function list_show(currentline, pageline) {
        ...
        d.each(assoc_array, function (n, m) {
            ...
            this_html += '<td ><i et="click tap:add_black"  class="list_edit iconfont icon-add" sh_title="SHpack.joinblack" title="' + SHpack.joinblack[nowLang] + '"></i></td>';
            this_html += '</tr>';
        })
    ...
    function filter_list(currentline, pageline) {
        var this_html = '';
        d("#black_info").empty();
        d.each(macfilter_info, function (n, m) {
            ...
            this_html += '<td class="text-center user_mac">' + m.src_mac.toUpperCase() + '</td>';
            this_html += '<td class="text-center user_num hide">' + m.real_num + '</td>';
            this_html += '<td class="text-center"><a et="click tap:del_black" class="list_edit iconfont icon-del" sh_title = "SHpack.removeblack" title="' + SHpack.removeblack[nowLang] + '"></a></td>';
            this_html += '</tr>';
        });
    ...
    et.add_black = function (evt) {
        var a = {};
        a.src_mac = d(evt).parents('tr').find('.user_mac').text().toLowerCase();
        a.operate = "add";
        set_config(a);
    };
    ...
    et.del_black = function (evt) {
        var a = {};
        a.list = d(evt).parents('tr').find('.user_num').text().toLowerCase() + ",";
        a.operate = "del";
        set_config(a);
    };

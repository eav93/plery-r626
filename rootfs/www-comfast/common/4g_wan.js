define(function(require, exports) {
	...
	var device, config, lan_info, radios_info, wan_info, wifis_info, setflag = 0,
		link_type = '';
	...
	function refresh_init() {
		...
		f.getguide(function(data) {
			if (data.errCode == 0) {
				...
				refresh_guide();
				displays();//Remember to call theinterface,otherwise your apn_auth subacript will always be zero
			}
		});
	}
	...
	function displays(){
		var apn_auth = d('#apn_auth').val()//get apn authetication ID
		if(apn_auth == 0){//if the APN authentication subscript is zero
			d("#usernamebox").addClass('siapals')
			d("#passwordbox").addClass('siapals')
		}else if(apn_auth != 0){//Otherwise, it is reversed
			d("#usernamebox").removeClass('siapals')
			d("#passwordbox").removeClass('siapals')
		}
	}
	displays()//Global call change event
	et.setmode = function() {
		...
		//When apN is selected with index 0
		...
		}else if(apn_auth != 0){//When APN is not 0
			if(d("input[name='addr_ip']:checked").val() == 'qmi' && d('#apn_detect').val() == 0) {
				var arg;
				if (lte_username == '' && lte_password == "") {
					...
					d("#lte_password").addClass('borError require')
				} else if(/[\u4E00-\u9FA5]/g.test(lte_password)){

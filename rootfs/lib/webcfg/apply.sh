...
for cfg in $CHANGED_CONFIGS ; do 
	[ $cfg = "network" ] && {
		...
		guide_login=`echo $net_chag | grep guidelogin`
		...
		[ -n "$guide_login" ] && {
			# login.user.guide=1, reload network
		...
		[ -n "$lanip_flag" ] && {
			...
			[ -f /tmp/sysinfo/noswitch ] || {
				...
				/sbin/reload_config
				[ -x /sbin/wifi ] && /sbin/wifi disassoc
				sleep 5
			}
	...
	[ $cfg = "dhcp" ] && {
		...
		[ -z "$network_exist" ] && {
			...
			/etc/init.d/dnsmasq restart
			login=`echo "$CHANGED_CONFIGS" |grep login`
			[ -z "$login" ] && {
				[ -x /sbin/wifi ] && /sbin/wifi disassoc
			}
		}
	...
	[ $cfg = "wireless" ] && {
		...
		is_mtk=`cat /tmp/sysinfo/driver_tpye 2>/dev/null`
		if [ "$is_mtk" == "mtk" ]; then
	...
	[ $cfg = "heart_beat" ] && {                                                                                                                                                             
		logger -t webcfg "Restarting heart_beat services..."
		/etc/init.d/heart_beat restart
		safe_exit
	} 
	...
	[ $cfg = "system" ] && {
		...
		[ $sys_chag = "auto_upgrade" ] && {
			[ -e /etc/init.d/fota_git_init ] && /etc/init.d/fota_git_init restart
		}

		[ $sys_chag = "manual_check" ] && {
			[ -e /etc/init.d/fota_git_init ] && /etc/init.d/fota_git_init stop
			[ -e /usr/bin/fota_git ] && /usr/bin/fota_git -d &
		}

		[ $sys_chag = "cancel_download" ] && {
			[ -e /etc/init.d/fota_git_init ] && /etc/init.d/fota_git_init stop
		}
	...
	[ $cfg = "openvpn" ] && {
		logger -t webcfg "......Restarting VPN server openvpnclient services..."
		uci commit
